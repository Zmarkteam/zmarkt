...
#define MY_REPORT_DISTANCE_EVT          0x0002
...
void zb_HandleOsalEvent( uint16 event )
{
  ...
  if ( event & MY_REPORT_DISTANCE_EVT )
  {
    ...
    pData[0] =  0x06;
    pData[1] =  0x02;
    pData[2] = 0xff & srf05Distance();
    zb_SendDataRequest(0, ID_CMD_REPORT, 3, pData, 0, AF_ACK_REQUEST, 0 );
    osal_start_timerEx( sapi_TaskID, MY_REPORT_DISTANCE_EVT, myWorkMode*1000 );
  }
}
/*********************************************************************
 * @fn      zb_HandleKeys
...
static int paramWrite(uint16 pid, byte *dat)
{
  ...
  case 0x0601:
    ...
    if (myWorkMode) {
      ...
      osal_stop_timerEx( sapi_TaskID, MY_REPORT_DISTANCE_EVT );
    }
    break;
  }
  return len;
