...
static byte lightstatus = 0; 

#define LIGHT01_DDR   P0DIR
#define LIGHT01_SBIT  P0_5
#define LIGHT01_BV    BV(5)

void light_init(void);
void light_onoff(uint8 st);

void light_init(void)
{
    LIGHT01_DDR |= LIGHT01_BV;
    lightstatus = 0;
    light_onoff(0);
}
...
{
  if (st == 0) {
   st( LIGHT01_SBIT = ACTIVE_LOW (1); );
  } else {
      st( LIGHT01_SBIT = ACTIVE_LOW (0); );
  }
}
