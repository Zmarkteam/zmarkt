...
#define My_SENSOR_CHECK             0x0002

/*********************************************************************
 * TYPEDEFS
 */
#define INF_PORT  P1
#define INF_SEL   P1SEL
#define INF_DIR   P1DIR
#define INF_BIT   0x01
#define INF_PIN   P1_0

void inf_init(void);
uint8 inf_check(void);

void inf_init(void)
{
  INF_SEL &= ~(INF_BIT);    /* Set pin function to GPIO */
  INF_DIR &= ~(INF_BIT);    /* Set pin direction to Input */
  
}

uint8 inf_check(void)
{
    //return (INF_PORT & INF_BIT);
      return INF_PIN;
}
...
static uint8 mySenserCheckDelay = 200;




static uint8 sEnable = 0x01; 
static uint8 sSensorData = 0;
...
void zb_HandleOsalEvent( uint16 event )
{
  ...
  if (event & My_SENSOR_CHECK) {
    ...
    osal_start_timerEx( sapi_TaskID, My_SENSOR_CHECK, mySenserCheckDelay );
  }
}
/*********************************************************************
 * @fn      zb_HandleKeys
...
static int paramWrite(uint16 pid, byte *dat)
{
  ...
  case 0x0401:
    ...
    if (sEnable != 0) {
      ...
      osal_stop_timerEx( sapi_TaskID, My_SENSOR_CHECK );
    }
    len = 1;
    break;
  }
